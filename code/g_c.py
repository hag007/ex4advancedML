...
input_shape = (784,) #(28, 28, 1)
# inputs = Input(shape=Input(shape=(original_dim,)), name='encoder_input')
...
d_h = Dense(intermediate_dim, activation='relu')(z)
outputs = Dense(np.size(input), activation='relu')(d_h)
# outputs = Reshape((tuple(list(shape)[1:])))(x)
